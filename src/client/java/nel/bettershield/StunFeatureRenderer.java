...
public class StunFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    private void renderStar(MatrixStack matrices, VertexConsumer vertexConsumer, float size, int light) {
        ...
        // --- 1.20.5 FIX: Pass 'entry' directly instead of 'entry.getNormalMatrix()' ---
        vertexConsumer.vertex(entry.getPositionMatrix(), -size, -size, 0.0F).color(255, 255, 255, 255).texture(0.0F, 1.0F).overlay(OverlayTexture.DEFAULT_UV).light(glowLight).normal(entry, 0.0F, 1.0F, 0.0F).next();
        vertexConsumer.vertex(entry.getPositionMatrix(), size, -size, 0.0F).color(255, 255, 255, 255).texture(1.0F, 1.0F).overlay(OverlayTexture.DEFAULT_UV).light(glowLight).normal(entry, 0.0F, 1.0F, 0.0F).next();
        vertexConsumer.vertex(entry.getPositionMatrix(), size, size, 0.0F).color(255, 255, 255, 255).texture(1.0F, 0.0F).overlay(OverlayTexture.DEFAULT_UV).light(glowLight).normal(entry, 0.0F, 1.0F, 0.0F).next();
        vertexConsumer.vertex(entry.getPositionMatrix(), -size, size, 0.0F).color(255, 255, 255, 255).texture(0.0F, 0.0F).overlay(OverlayTexture.DEFAULT_UV).light(glowLight).normal(entry, 0.0F, 1.0F, 0.0F).next();
    }
}